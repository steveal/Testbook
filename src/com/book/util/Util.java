...
public class Util {
	...
	public static void DownloadBook(Book b, String store) {
		List<BookChapter> chapterList = getChapterList(b.getBookAddress());
...
//		b.setChapterList(chapterList);
		///////
		String indexHtml = createBookIndexPage(store, b);
		// produce index page
		writeFile(store + "index.html", indexHtml);
		...
		for (int i = 0; i < chapterList.size();) {
			...
			if (toIndex < chapterList.size()) {
				dltList.add(new DownloadThread(store, b.getBookAddress(),
	...
	public static String getChapter(String url, BookChapter bc) {
		String response = Util.getPageResponse(url + bc.getChapterFileName());
...
//				+ "}" + "\n"
		...
		String html = "<html>" + "<head>" + "<title>"
				...
				+script
				+ "</head>"
				+ "<body bgcolor=\"#C7EDCC\"   style=\"line-height:1.5\">"
				+ "<a href=\"" + bc.getPreviousChapterFileName() + "\">Previous Chapter" + "</a>" + "<br/>"
				+ "<a href=\"" + bc.getNextChapterFileName() + "\">Next Chapter" + "</a>" +  "<br/>"
				+ "<a href=\"index.html\">Index</a>" +  "<br/>"
				+ "<h1>" + chapterName + "</h1>" + "<p>" + content + "</p>"
				+ "<a href=\"" + bc.getPreviousChapterFileName()
				+ "\">Previous Chapter" + "</a>" + "<br/>" + "<a href=\""
				+ bc.getNextChapterFileName() + "\">Next Chapter" + "</a>"
				+ "</body>" + "</html>";

		return html;
	}

	public static List<BookChapter> getChapterList(String url) {
		List<BookChapter> bookChapterList = new ArrayList<BookChapter>();
		String reponse = getPageResponse(url);
		...
		NodeFilter nfChapterList = new AndFilter(new TagNameFilter("a"),
				new HasParentFilter(new TagNameFilter("dd")));
		NodeList nlChapterNmae;
		try {
			...
			for (Node node : nodelist) {
				...
				BookChapter bc = new BookChapter(node.toPlainTextString(),
						href, "");
		...
		bookChapterList.get(0).setNextChapterFileName(
				bookChapterList.get(1).getChapterFileName());
		int length = bookChapterList.size();
		for (int i = 1; i < length - 1; i++) {
			bookChapterList.get(i).setNextChapterFileName(
					bookChapterList.get(i + 1).getChapterFileName());
			bookChapterList.get(i).setPreviousChapterFileName(
					bookChapterList.get(i - 1).getChapterFileName());
		}
		bookChapterList.get(length - 1).setPreviousChapterFileName(
				bookChapterList.get(length - 2).getChapterFileName());
